...
def load_point_cloud(file_path):
    ...
    try:
        ...
        if len(np.asarray(point_cloud.points)) == 0:
            print(f"Error: The loaded point cloud from {file_path} has no points.")
            return None
        return point_cloud
...
def align_to_manhattan(point_cloud, visualize=True):
    ...
    if point_cloud is None or len(np.asarray(point_cloud.points)) == 0:
        print("Error: Invalid point cloud for alignment.")
        return None, np.eye(3), np.zeros(3)
    
    # Copy original point cloud data
    points = np.asarray(point_cloud.points).copy()
    has_colors = point_cloud.has_colors()
    original_colors = np.asarray(point_cloud.colors).copy() if has_colors else None
    
    # Simple outlier removal
    try:
        cl, _ = point_cloud.remove_statistical_outlier(nb_neighbors=20, std_ratio=2.0)
        if len(np.asarray(cl.points)) == 0:
            print("Warning: Outlier removal eliminated all points. Using original point cloud.")
            cl = point_cloud
    except Exception as e:
        print(f"Warning: Error during outlier removal: {e}. Using original point cloud.")
        cl = point_cloud
    
    # Get cleaned points
    points = np.asarray(cl.points).copy()
    ...
    try:
        ...
        pca.fit(points)
        principal_axes = pca.components_  # eigenvectors
        variances = pca.explained_variance_  # eigenvalues
        
        print("PCA Variances:", variances)
        print("Principal Axes:")
        print(principal_axes)
        
        # For indoor scenes, the axis with smallest variance is typically vertical (Z)
        ...
        # Get the other two axes (indices other than up_idx)
        other_indices = [i for i in range(3) if i != up_idx]
        x_axis = principal_axes[other_indices[0]]
        y_axis = principal_axes[other_indices[1]]
        ...
        # Create rotation matrix (rows are the principal axes)
        ...
        return point_cloud, np.eye(3), centroid
    
    # Apply transformation
    aligned_points = np.dot(points - centroid, R.T) + centroid
    ...
    if has_colors and original_colors is not None:
        if len(original_colors) == len(aligned_points):
            aligned_result.colors = o3d.utility.Vector3dVector(original_colors)
        else:
            print("Warning: Color array size mismatch. Using default coloring.")
            aligned_result.paint_uniform_color([0, 0.651, 0.929])  # Blue
    ...
    if visualize:
        try:
            ...
            if has_colors and original_colors is not None:
                ...
                original_viz.paint_uniform_color([1, 0.706, 0])  # Orange
            ...
            if has_colors and original_colors is not None:
                ...
                aligned_viz.paint_uniform_color([0, 0.651, 0.929])  # Blue
...
def save_transformation(R, centroid, output_file):
    ...
    try:
        ...
        with open(output_file, 'w') as f:
            ...
            # Save R.T which is used for transforming points
            R_save = R.T
            for row in R_save:
...
def main():
    ...
    if point_cloud is None:
        print("Error: Failed to load point cloud. Exiting.")
        return
    ...
    if aligned_cloud is None:
        print("Error: Alignment failed. Exiting.")
        return
    ...
    try:
        ...
        print(f"Successfully saved aligned point cloud to {output_path}")
    except Exception as e:
    ...
    if save_transformation(R, centroid, transform_path):
